...
describe("Server", () => {
    ...
    describe("GET /getHeight when all parameters are good", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/Heights/getHeight?height=175&quantity=50", (error, response, body) => {
                data.error = error;
        ...
        it("Body has to contain appropriate data", () => {
            expect(data.body).toContain({ "valuenum": 175 });
        });
        it("Body has not to contain inappropriate data", () => {
            expect(data.body).not.toContain({ "valuenum": 190 });
        });
    ...
    describe("GET /getHeight when quantity is the only parameter", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/Heights/getHeight?quantity=50", (error, response, body) => {
                data.error = error;
                data.status = response.statusCode;
                data.body = JSON.parse(body);
                done();
    ...
    describe("GET /getHeight when quantity has a wrong value", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/Heights/getHeight?quantity=-5", (error, response, body) => {
                data.error = error;
                data.status = response.statusCode;
                data.body = JSON.parse(body);
                done();
    ...
    describe("GET /getHeight when required parameter (quantity) is missing", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/Heights/getHeight?height=175", (error, response, body) => {
                data.error = error;
