...
describe("Server", () => {
    ...
    describe("GET /getDiastolicBloodPressures when all parameters are good", () => {
        ...
        it("Body has to contain appropriate data", () => {
            expect(data.body).toContain({ "valuenum": 80 });
        });
    ...
    describe("GET /getDiastolicBloodPressures when quantity is the only parameter", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/DiastolicBloodPressures/diastolic?quantity=10", (error, response, body) => {
                data.error = error;
                data.status = response.statusCode;
                data.body = JSON.parse(body);
                done();
    ...
    describe("GET /getDiastolicBloodPressures when quantity has a wrong value", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/DiastolicBloodPressures/diastolic?quantity=-5", (error, response, body) => {
                data.error = error;
                data.status = response.statusCode;
                data.body = JSON.parse(body);
                done();
    ...
    describe("GET /getDiastolicBloodPressures when required parameter (quantity) is missing", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/DiastolicBloodPressures/diastolic?diastolicBloodPressure=80", (error, response, body) => {
                data.error = error;
