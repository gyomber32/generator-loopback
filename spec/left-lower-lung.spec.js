...
describe("Server", () => {
    ...
    describe("GET /getLeftLowerLungs when all parameters are good", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/LeftLowerLungs/LLL?lll=Clear&quantity=10", (error, response, body) => {
                data.error = error;
    ...
    describe("GET /getLeftLowerLungs when quantity is the only parameter", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/LeftLowerLungs/LLL?quantity=10", (error, response, body) => {
                data.error = error;
                data.status = response.statusCode;
                data.body = JSON.parse(body);
                done();
    ...
    describe("GET /getLeftLowerLungs when quantity has a wrong value", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/LeftLowerLungs/LLL?quantity=-5", (error, response, body) => {
                data.error = error;
                data.status = response.statusCode;
                data.body = JSON.parse(body);
                done();
    ...
    describe("GET /getLeftLowerLungs  when required parameter (quantity) is missing", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/LeftLowerLungs/LLL?lll=Clear", (error, response, body) => {
                data.error = error;
