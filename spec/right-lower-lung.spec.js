...
describe("Server", () => {
    ...
    describe("GET /getRightLowerLungs when all parameters are good", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/RightLowerLungs/RLL?rll=Clear&quantity=10", (error, response, body) => {
                data.error = error;
    ...
    describe("GET /getRightLowerLungs when quantity is the only parameter", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/RightLowerLungs/RLL?rll=Clear&quantity=10", (error, response, body) => {
                data.error = error;
    ...
    describe("GET /getRightLowerLungs when quantity has a wrong value", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/RightLowerLungs/RLL?rll=Clear&quantity=-5", (error, response, body) => {
                data.error = error;
                data.status = response.statusCode;
                data.body = JSON.parse(body);
                done();
    ...
    describe("GET /getRightLowerLungs when required parameter (quantity) is missing", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/RightLowerLungs/RLL?rll=Clear", (error, response, body) => {
                data.error = error;
