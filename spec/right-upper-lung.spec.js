...
describe("Server", () => {
    ...
    describe("GET /getRightUpperLungs when all parameters are good", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/RightUpperLungs/RUL?rul=Clear&quantity=10", (error, response, body) => {
                data.error = error;
    ...
    describe("GET /getRightUpperLungs when quantity is the only parameter", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/RightUpperLungs/RUL?quantity=10", (error, response, body) => {
                data.error = error;
                data.status = response.statusCode;
                data.body = JSON.parse(body);
                done();
    ...
    describe("GET /getRightUpperLungs when quantity has a wrong value", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/RightUpperLungs/RUL?quantity=-5", (error, response, body) => {
                data.error = error;
                data.status = response.statusCode;
                data.body = JSON.parse(body);
                done();
    ...
    describe("GET /getRightUpperLungs when required parameter (quantity) is missing", () => {
        var data = {};
        beforeAll((done) => {
            Request.get("http://localhost:3100/api/RightUpperLungs/RUL?rul=Clear", (error, response, body) => {
                data.error = error;
